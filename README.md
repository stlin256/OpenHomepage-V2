...
- **Responsive Image Optimization** — Production builds create multiple WebP + AVIF widths, serve AVIF first via `<picture>` when the browser supports it (WebP as fallback), and emit layout-derived 1x / 2x / 3x candidates so the browser selects the exact density it needs without over-fetching desktop-sized files. Aggressive idle prefetch covers language alternates and same-language tabs plus their AVIF candidates without a byte cap, while prefetch-only Speculation Rules warm hover targets in Chromium. Fetched HTML feeds a shared in-memory cache, so language switches are near-instant; originals and `-full` lightbox sources remain excluded from preloading.
