...
- **配置**——站点/资料/链接/站点图标（favicon：素材库 svg/png/ico 选择，或直接上传任意图片自动居中裁方、转换生成 180×180/32×32 PNG 并写回配置）、背景音乐（开关、素材库文件选择、音量滑块）、**页脚**（默认开启，开关 + 双语文本，支持 `[文字](链接)` 内联链接）、GitHub（用户名、贡献图开关、pinned 增删与上移下移排序）、RSS（源的 mode/latest/weight/cover 与精选文章子列表）、流式块定义，以及可拖拽排序的 `home.layout`。
