...
- **本地可视化编辑器（PC）**——内置本地独立编辑器（`npm run admin`，基于 Milkdown），支持所见即所得、源码直写与双栏实时预览三态切换，配备自动保存、版本快照回滚与一键数据打包。
...
- **三种编辑模式**：所见即所得（WYSIWYG 交互预览卡片）、Markdown 源码直写（双向实时同步）与双栏实时预览（左侧编辑，右侧同步刷新 iframe）。
