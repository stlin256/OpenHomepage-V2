...
- **配置**——站点/资料/链接/站点图标（favicon，素材库 svg/png/ico 选择，缺省用内置默认）、背景音乐（开关、素材库文件选择、音量滑块）、GitHub（用户名、贡献图开关、pinned 增删与上移下移排序）、RSS（源的 mode/latest/weight/cover 与精选文章子列表）、流式块定义，以及可拖拽排序的 `home.layout`。
