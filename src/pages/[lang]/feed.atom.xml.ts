...
export function getStaticPaths() {
  ...
  if (!cfg.enabled || !cfg.formats.includes('atom')) return [];
  return feedLangParams(loadPages(dataDir), site.site.language).map((lang) => ({ params: { lang } }));
}

export const GET: APIRoute = async ({ site, params }) => {
  const doc = await buildFeedDocument('atom', { siteUrl: site, requestedLang: params.lang });
  if (doc === null) return new Response('', { status: 404 });
  return new Response(doc, { headers: { 'Content-Type': 'application/atom+xml; charset=utf-8', 'Cache-Control': 'public, max-age=3600' } });
};
