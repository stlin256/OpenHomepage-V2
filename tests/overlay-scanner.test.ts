...
describe("main：编辑模式下超链接点击拦截", () => {
  ...
  it("点击页面内普通超链接（站内/外链）均调用 preventDefault，避免意外跳转", () => {
    document.body.innerHTML = [
      "<nav class=\"site-nav\"><a id=\"nav-link\" href=\"/research/\">研究</a></nav>",
      "<div class=\"markdown-body\">",
      "  <p data-oh-src=\"pages/zh/index.md:0,10\">正文 <a id=\"content-link\" href=\"https://example.com\">外链</a></p>",
      "</div>",
      "<footer data-oh-cfg=\"footer.text@zh\"><p><a id=\"footer-link\" href=\"/about\">关于</a></p></footer>",
    ].join("");
    initOverlay(document);

    const navLink = document.querySelector<HTMLAnchorElement>("#nav-link")!;
    const clickEvt1 = new MouseEvent("click", { bubbles: true, cancelable: true });
    navLink.dispatchEvent(clickEvt1);
    expect(clickEvt1.defaultPrevented).toBe(true);

    const contentLink = document.querySelector<HTMLAnchorElement>("#content-link")!;
    const clickEvt2 = new MouseEvent("click", { bubbles: true, cancelable: true });
    contentLink.dispatchEvent(clickEvt2);
    expect(clickEvt2.defaultPrevented).toBe(true);

    const footerLink = document.querySelector<HTMLAnchorElement>("#footer-link")!;
    const clickEvt3 = new MouseEvent("click", { bubbles: true, cancelable: true });
    footerLink.dispatchEvent(clickEvt3);
    expect(clickEvt3.defaultPrevented).toBe(true);
  });

  it("auxclick（中键等）在页面超链接上调用 preventDefault", () => {
    document.body.innerHTML = "<a id=\"mid-link\" href=\"/research/\">链接</a>";
    initOverlay(document);
    const link = document.querySelector<HTMLAnchorElement>("#mid-link")!;
    const auxEvt = new MouseEvent("auxclick", { bubbles: true, cancelable: true, button: 1 });
    link.dispatchEvent(auxEvt);
    expect(auxEvt.defaultPrevented).toBe(true);
  });

  it("overlay 自身控件（如顶栏 ←后台 链接）不拦截默认跳转", () => {
    (window as unknown as Record<string, unknown>).__OH_ADMIN_ORIGIN__ = "http://127.0.0.1:4174";
    vi.stubGlobal("fetch", vi.fn(async () => Promise.reject(new Error("offline"))));
    document.body.innerHTML = "<p data-oh-src=\"pages/zh/index.md:0,5\">甲</p>";
    initOverlay(document);
    ...
    const auxEvt = new MouseEvent("auxclick", { bubbles: true, cancelable: true, button: 1 });
    backLink.dispatchEvent(auxEvt);
    expect(auxEvt.defaultPrevented).toBe(false);
  });
});
