...
- 音频始终 `preload="none"`，不参与首屏加载；`autoplay` 或 localStorage 记忆为播放态时，页面初始化即机会性调用 `play()`。
- 自动播放策略：浏览器允许时首屏直接开播；若自动播放被拦截，则等首次用户交互（click/keydown）后调用 `load()` 并恢复播放。localStorage 记住用户上次播放/暂停；用户点过播放按钮（本身是手势）立即开播。
